  ...
  return (
    <main className="min-h-screen pt-32 pb-24 px-6 md:px-12 lg:px-24">
      <div className="max-w-4xl mx-auto">
        <div className="mb-16">
          <Link href="/" className="font-mono text-sm text-gray-500 hover:text-[var(--color-neon-cyan)] transition-colors mb-6 inline-block">
            ← Back to Home
          </Link>
          ...
          <p className="text-gray-400 text-lg max-w-2xl leading-relaxed">
            A behind-the-scenes look at the projects I've built — the problems, the decisions, and the outcomes.
          </p>
        </div>
        ...
        <div className="space-y-8">
          {posts.map((post, index) => {
            ...
            const CardWrap = isDraft ? "div" : Link;

            return (
              <CardWrap
                key={index}
                href={isDraft ? undefined : post.link}
                className={`block bg-[#111111] border border-[#1e1e1e] p-8 relative rounded-sm transition-all duration-300 ${
                  isDraft
                    ? "opacity-50 cursor-default"
                    : "hover:border-[rgba(0,229,255,0.3)] hover:-translate-y-1 block cursor-pointer"
                }`}
              >
